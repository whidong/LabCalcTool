...
이 프로그램은 실험실에서 NH4, SS, VSS 농도를 계산하기 위한 도구입니다. <br>
여러 시료의 적정량을 입력하면, 자동으로 NH4, SS, VSS 농도를 계산해줍니다.
...
<details>
 ...
 ```
  - -F, --onefile
     ...
     -  하나의 독립 실행 파일로 패키징 합니다.
  - -D, --onedir 
     -  기본 옵션, 여러 파일로 분리하여 저장합니다.
...
pyinstaller --icon=파일명.ico scriptname.py
 ```
  ...
  - -add-date "SRC;DEST" (windows) / -add-date "SRC:DEST" (unix) 
     - 외부 데이터 파일을 포함합니다.
...
- --clean   
   - 빌드 폴더와 캐시를 지우고 빌드합니다.
     
- --upx-dir <UPX_DIR>
   - UPX 압축기를 사용해 실행 파일을 더 작게 만듭니다.
